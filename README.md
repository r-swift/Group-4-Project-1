...
Using our clean and merged data set we ran a regression with EPS and Annualized % Growth with regards to ESG Total Score. The resulting correlation from both studies was rather weak with Annualized % Growth having a correlation of 0.13 and EPS having a correlation of 0.2. Based on these scores it shows that while there is a slight positive correlation between a companies ESG Total Score and their Earning per Share the correlation is not meaningful. For more detailed technical Analysis please see the ESG.ipynb Jupyter Notebook.
